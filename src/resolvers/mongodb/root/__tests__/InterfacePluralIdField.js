...
let graphql;
let db;
const dbName = 'InterfacePluralIdFieldTest';

describe('Mongo Resolvers / root / InterfacePluralIdField:', function () {
  ...
  it('Returns a single object from one interface implementation', async () => {
    const message = `
      query TestCase {
        person(id: "-K5W9B5PtuqN3eOVemge-VDGCFP55") {
          ... on Node {
            id
          }
          name
  ...
  it('Returns a single object from another implementation', async () => {
    const message = `
      query TestCase {
        person(id: "-K5Wn3VQfTc_r7tD81hu-TFEKI13KFI") {
          ... on Node {
            id
          }
          name
  ...
  it('Returns multiple objects across implementations', async () => {
    const message = `
      query TestCase {
        person(id:
  ...
  it('Returns multiple objects from the same implementation', async () => {
    const message = `
      query TestCase {
        person(id:
  ...
  it('Returns null if object type is incorrect', async () => {
    const message = `
      query TestCase {
        person(id: ["-K5WwnwPBaL7PWFFgttp-SFK", "-K5WwrfJ8P312KiyX4z0-WFF"]) {
          ... on Node {
            id
          }
          name
